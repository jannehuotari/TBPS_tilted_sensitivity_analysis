% This function produces runs x 24 matrix, with 1:12 being the z_coverages,
% and 13:24 being the z_overlaps. 
% layer is specified as a number (ex. layer = 1)
% modules specifies inner or outer modules (modules = 'inner'; or modules = 'outer';)
% z, r and tilt change variables are runs x 12 arrays where changes are specified for each module.
% runs_n specifies the number of times results will be run and depends on
% how large the arrays of the change variables are.

% To run the z-coverages once at nominal values, specify change arrays as
% 1:12 arrays filles with zeros (z_change = zeros(1,12) and so on..) and
% runs_n as 1.
function [z_cov_results] = zcov(layer, modules, z_change, r_change, tilt_change, runs_n)
...
z_cov = zeros(1,12);
z_overlap = zeros(1,12);
% Initialize results, 1:12 = z-coverages of modules, 13-24 = z-overlaps of
% modules:
z_cov_results = zeros(runs,24);
...
for g = 1:runs
    % Apply changes to nominal values
    for t = 1:13
        if t == 1
            r_s1(t) = r_s1_i(t);
            r_s2(t) = r_s2_i(t);
    ...
    % Loop for all rings in tilted section:
    for k = 1:12
        
        % Calculate z & y of points 1, 2, 3 & 4 (see module sensitivity analysis report in sharepoint for further clarification. Especially section on z-coverage
        % and appendix A.)
    ...
    z_cov_results(g,13:24) = z_overlap;
end
