...
% Testiin
layer   = 3; % Layer to analyze
modules = 'outer'; % "inner" or "outer" modules to analyze
z_change = linspace(0,30,2000);
r_change = linspace(0,0,2000);
tilt_change = linspace(0,deg2rad(3),2000);
...
z_m_layer1_inner = [128.763, 171.150, 217.118, 267.987, 315.184, 374.203, 443.696, 526.772, 611.087, 720.246, 850.617, 1004.205, 1182.332];
...
% Outer module radius:
r_m_layer1_outer = [241.337 265, 265, 265, 259, 259, 259, 259, 254, 254, 254, 254, 254];
% Outer module position in z:
z_m_layer1_outer = [128.763, 180.946, 226.914, 277.784, 327.101, 386.121, 455.614, 538.690, 623.389, 732.548, 862.919, 1016.507, 1194.634];
...
% Inner module position in z:
z_m_layer2_inner = [222.566, 270.888, 321.982, 378.406, 431.314, 495.745, 567.262, 646.108, 725.236, 820.642, 927.596, 1048.134, 1181.542];
...
% Outer module position in z:
z_m_layer2_outer = [222.566, 278.394, 329.488, 385.911, 440.314, 504.745, 576.262, 655.108, 736.439, 831.844, 938.798, 1059.336, 1192.744];
...
% Inner module position in z:
z_m_layer3_inner = [315.734, 362.750, 415.650, 472.100, 532.300, 596.600, 664.750, 727.800, 804.700, 887.800, 978.100, 1075.000, 1179.300];
...
% Outer module position in z:
z_m_layer3_outer = [315.734, 370.844, 423.744, 480.194, 540.394, 604.694, 672.844, 737.334, 814.234, 897.334, 987.634, 1084.534, 1188.834];
...
for k = 1:12
    ...
    while z_cov(k) > 70
        ...
        if z_cov(k) > 70
            if k == 1
                %r_s1(k+1) = r_s1_i(k+1) - r_change(n);
                %r_s2(k+1) = r_s2_i(k+1) - r_change(n);

                z_s1(k+1) = z_s1_i(k+1) + z_change(n);
                z_s2(k+1) = z_s2_i(k+1) + z_change(n);

                n = n + 1;
            elseif rem(k,2) == 0
                %r_s1(k) = r_s1_i(k) + r_change(n);
                %r_s2(k) = r_s2_i(k) + r_change(n);

                z_s1(k) = z_s1_i(k) - z_change(n);
                z_s2(k) = z_s2_i(k) - z_change(n);
        ...
        elseif z_cov(k) < 70
            if k == 1
                z_tol(1,1) = z_change(n);
                r_tol(2,1) = r_change(n);
                n = 1;
            elseif k ~= 1
                z_tol(1,k) = z_change(n);
                z_tol(2,k-1) = z_change(n);
                r_tol(2,k) = r_change(n);
                r_tol(1,k-1) = r_change(n);
                n = 1;
            end
        end
    end
