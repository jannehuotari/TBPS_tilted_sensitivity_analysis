function [r_tol] = zcov_rtol(layer, modules, z_change, r_change, allowed_cov, ratio)
...
for k = 1:12
    ...
    z_s2 = z_s2_i;
    if k == 1
        z_s1(k+1) = z_s1_i(k+1) + z_change(1,k)*ratio;
        z_s2(k+1) = z_s2_i(k+1) + z_change(1,k)*ratio;
    elseif k ~= 1
        z_s1(k+1) = z_s1_i(k+1) + z_change(1,k)*ratio;
        z_s2(k+1) = z_s2_i(k+1) + z_change(1,k)*ratio;
        
        z_s1(k) = z_s1_i(k) - z_change(2,k-1)*ratio;
        z_s2(k) = z_s2_i(k) - z_change(2,k-1)*ratio;
    end
    while z_cov(k) > allowed_cov
